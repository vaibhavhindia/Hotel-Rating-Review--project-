...
if st.button("Analyze"):
    ...
    finalPrediction = "The review is positive 😀" if predictedVal=="0" else "The review is negative 😔"
      
    st.markdown("<p class='big-font'>{}</p>".format(finalPrediction),unsafe_allow_html=True)
    prdictionDist = model_sentiment.predict(tfIdfText)

    resN = get_percentage(prdictionDist[0])
    resP = get_percentage(prdictionDist[1])
